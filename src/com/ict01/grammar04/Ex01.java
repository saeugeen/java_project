...
class Ex01{
	...
	//swich ~ case문 : if 문과 같은 분기문
	// if문의 조건식은 boolean형, 비교연산, 논리연산
	//switch문은 byte, short, int, char , String 일때 사용
	//범위가 나오면 기본적으로는 if문을 사용한다.
	//switch문은 100% if문으로 변환이 가능하다. (반대는 상객해봐야함)
	//형식 : 
	//	switch(인자값(char, int , String)){
	//	case 조건값1 : 인자값이 조건값1과 같을때; break;
	//	case 조건값2 : 인자값이 조건값2과 같을때; break;
	//	case 조건값3 : 인자값이 조건값3과 같을때; break;
	//	default : 조건값 1,2,3 모두 요구하는 값이 아닌때, 즉 나머지
	//	}
	// ** 주의사항 ** break문이 없으면 다음 break문을 만날때까지 모든 실행문을 실행한다.
	//break문은 해당 switch문을 빠져나가는 역할을 한다.

	//char k1 가 A이면 아프리카 B이면....
	char k = 'c';
	String str = "";
	switch(k){
	...
	//char k1 가 A,a이면 아프리카 B,b이면....
	char k1 = 'c';
	String str1 = "";
	switch(k1){
		case 'A' : 
		case 'a' : str1 = "아프리카"; break;
		case 'B' : 	
		case 'b' : str1 = "브라질"; break;
		case 'C' :
		case 'c' : str1 = "캐나다"; break;
		default :str1 ="한국";
	}
