...
public class Ex02 {
	public static void main(String[] args) {
		//Set 인터페이스를 상속 받은 클래스 : HashSet, TreeSet
		...
		//	컬렉션 생성
		//	HashSet<제네닉타입 = 객체타입 = 같은 클래스> 참조변수 = new HashSet<제네닉타입>();
		//	HashSet<제네닉타입 = 객체타입 = 같은 클래스> 참조변수 = new HashSet<>();
		...
		System.out.println(h1);
		//하니씩 꺼내서 사용하기
		// 1. 개선된 for문(일반 for문은 사용 못함 (인데스가 없음))
		// 2. iterator()
		for (String k : h1) {
			String msg = k;
		...
		while (it.hasNext()) {	//hasNext() : 다음객체가 존재하면 true, 존재하지 않으면 false
			String msg = (String) it.next();	//next() : 다음객체를 return 하고 다음객체로 이동
			System.out.println(msg);
		}
